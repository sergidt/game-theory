...
import {  Color, Group, Material, Mesh, MeshBasicMaterial, MeshStandardMaterial, ShapeGeometry } from 'three';
...
export class Board {
    ...
    roundBigDark1 = injectLoader(() => GLTFLoader, () => '/assets/round-big-1.glb', {
      onLoad:  ({scene}: {scene: Group}) => setMaterialColor(scene, '42407D')
    });
    ...
    roundBigLight1 = injectLoader(() => GLTFLoader, () => '/assets/round-big-1.glb', {
      onLoad:  ({scene}: {scene: Group}) => setMaterialColor(scene)
    });
...
function setMaterialColor(scene: Group, color: string = 'FEEEAD')  {
  console.log(scene);
